...
def _load_uuid_utils() -> UUIDBackend:
    ...
    uuid.UUID
    return UUIDBackend(name="uuid-utils", uuid7=uuid.uuid7, UUID=uuid.UUID)


def _load_uuid6() -> UUIDBackend:
    import uuid6  # type: ignore
    return UUIDBackend(name="uuid6", uuid7=uuid6.uuid7, UUID=uuid6.UUID)
...
def get_uuid_backend() -> UUIDBackend:
    ...
    if forced:
        ...
        if forced not in ("uuid-utils", "uuid6"):
            raise RuntimeError(
                f"Invalid TYPEID_UUID_BACKEND={forced!r}. "
                "Allowed values: 'uuid-utils' or 'uuid6'."
            )
        try:
    ...
    try:
        ...
        raise RuntimeError(
            "No UUIDv7 backend available. Install one of: uuid-utils (recommended) or uuid6."
        ) from e
